...
#===================3.9针对去背景方式的不同，创建插值背景算法=====================
    ...
    def create_interpolation_back(self,start_back1,end_back1,start_back2,end_back2,rotation_sampling=False):
        ...
        if rotation_sampling:
            interpolation_num=sample_data.shape[1]/di.global_class_num
...
#============================3.10去插值背景=====================================
    ...
    def de_interpolation_back(self,interpolation_back=False):
        ...
        per_sample_num=data.shape[1]/di.global_class_num
            ...
            while j<data.shape[0]:
                ...
                if interpolation_back:
                    ...
                    if back.iloc[j,back_col]==0 or data.iloc[j,i]==0:
                        ...
                        deback.iloc[j,i]=log(back.iloc[j,back_col]/data.iloc[j,i])
                    j+=1
                    back_col+=1
                #启动去均值背景的去背景方式
                else: 
    ...
    def deback(self,sample_data,back_data,debacked_data,category_back=False):
        ...
        while i<data.shape[1]:
            ...
            if category_back == True:
                ...
                k=int(i//(data.shape[1]/di.global_class_num))
            #否则去综合背景    
            else:
                k=0
            while j<data.shape[0]:      
                ...
                if back.iloc[j,k]==0 or data.iloc[j,i]==0:
                    ...
                    #deback.iloc[j,i]=math.log(back.iloc[j,back_col]/data.iloc[j,i])
                    deback.iloc[j,i]=log(back.iloc[j,k]/data.iloc[j,i])
                    #print(k)
                j+=1
            j=0
            i+=1
...
#        print(pca.components_)
        ...
        return pca.explained_variance_ratio_,sumn
#=================================5.2IPCA======================================    
        #增量PCA降维算法（IPCA）。
    def IPca(self):
        data_set=pd.read_csv(self.data_set_name,header=None,index_col=None)
        ...
        data_set=pd.DataFrame(data_set)
        data_set.to_csv(self.Dred_data,header=False,index=False)
        return ipca.explained_variance_ratio_,sumn
#================================5.3NMF========================================    
    #非负矩阵分解降维算法.
    def Nmf(self,):
        data_set=pd.read_csv(self.data_set_name,header=None,index_col=None)
        ...
        data_set=pd.DataFrame(data_set)
        data_set.to_csv(self.Dred_data,header=False,index=False)
        ret_info_1="NMF does not exist The interpretability of each component."
        ret_info_2="Does not exit SUM too."
        return ret_info_1,ret_info_2     
#==============================5.4T-SNE========================================    
    #流形学习降维算法（非线性降维）.
#    def Tsne(self,):
#        data_set=pd.read_csv(self.data_set_name,header=None,index_col=None)
...
#        data_set=pd.DataFrame(data_set)
#        data_set.to_csv(self.Dred_data,header=False,index=False)
#        ret_info_1="T-SNE does not exist The interpretability of each component."
#        ret_info_2="Does not exit SUM too."
#        return ret_info_1,ret_info_2
#==============================5.5.LDA=========================================    
    #线性判别分析降维算法.
    def Lda(self):
        ...
        data_set=pd.DataFrame(data_set)
        data_set.to_csv(self.Dred_data,header=False,index=False)
        ret_info_1="LDA does not exist The interpretability of each component."
        ret_info_2="Does not exit SUM too."
        return ret_info_1,ret_info_2
...
#========================6.1splitting,数据集划分================================   
    ...
    #test_set_ratio为测试集所占比例
    def split_data_set(self,test_set_ratio,binarize_label=False):
        #读取pca降维后的数据集、对应标签
...
#            label=label_binarize(label,classes=list(range(0,SampleInformation().class_num)))   
            label=label_binarize(label,classes=list(range(0,di.global_class_num)))   
        #数据集分割成训练集和测试集(后续考虑采用k折交叉验证法分割数据集)。
        #测试集占20%，shuffle选择是否打乱顺序（不打乱则效果很差）
        self.x_train,self.x_test,self.y_train,self.y_test=train_test_split(
                data,label,test_size=test_set_ratio,shuffle=True)
        return 0
...
#---------------------6.3.2基于混淆矩阵评估，标签编码----------------------------    
    ...
    def confusion_matrix_eval(self,):
        ...
        cr=classification_report(self.y_test,self.y_predict,
                                 target_names=di.global_class_names)
        print("\n3.自动评估报告：")
        print(cr)
        #3.cohen-kappa系数
...
#-------------------------6.3.3画ROC和AUC曲线，独热编码-------------------------
    ...
    def draw_ROC_curves(self,):
        category_num=di.global_class_num
        if category_num ==2:
...
#======6.4综合以上{splitting，training、evaluation}三部分函数，便于直接调用=======
    ...
    #训练
    def training(self,):
        self.split_data_set(test_set_ratio=di.test_set_ratio,binarize_label=False)
        #将分类器训练过程和评估参数生成过程分离，使后续重构更简单
        self.classifiers() 
        train_result=self.output_accuracy()
        return train_result
    #评估
    def evaluation(self,K_Fold=10):
        #self.CT_predict()
        CM_output=CV_output=[]
        #根据GUI状态选择部分评估方式运行
        if di.evaluation_botton_state[0] == 1:
            CM_output=self.confusion_matrix_eval()
            self.draw_confusion_matrix()
            self.draw_confusion_matrix(normalize=True,)
            
        if di.evaluation_botton_state[1] == 1:
            self.draw_ROC_curves()
            
        if di.evaluation_botton_state[2] == 1:
            self.draw_PR_curves()
            
        if di.evaluation_botton_state[3] == 1:
            CV_output=self.cross_validation(k_fold=K_Fold)
        return CM_output,CV_output
...
class Drawing():
    ...
    def __init__(self,):
        ...
        #self.sample_names=di.global_class_names
...
#=================10.1定义通用绘图函数，用于绘制背景图和光谱图====================     
    def draw_data(self,csv_name,title,xlabel="Channel",ylabel="Number of photons",save_name="test",cnum=0):
        ...
        if data.shape[1]==1:
            ...
            if cnum==0:
                #只有平均值类的数据需要标出标注          
                plt.legend(di.global_class_names)
        #设置图片外围信息
